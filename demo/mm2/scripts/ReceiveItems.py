...
SOURCE_TOPIC= os.getenv('KAFKA_TOPIC','items')
TOPIC_POLL_S=30
...
def parseArguments():
    nb_records = 1
    if len(sys.argv) < 2:
        sys.exit("Error - Usage: python ReceiveItems.py [number_of_records]")
    else:
...
if __name__ == "__main__":
    ...
    print("--------- Start Consuming Items --------------")
    consumer_conf = getConsumerConfiguration("ItemConsumer", True)
